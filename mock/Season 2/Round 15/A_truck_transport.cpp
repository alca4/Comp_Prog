...
{
    in[a] = ++T;
    for (int j = 1; j < 32; j++) lift[a][j] = lift[lift[a][j - 1]][j - 1];
...
int main()
{
    ...
    for (int i = 1; i <= M; i++) combine(edgelist[i]);
    
    for (int i = 1; i <= N; i++) if (lift[i][0] != i) nbs[lift[i][0]].pb(i);
    for (int i = 1; i <= N; i++) if (lift[i][0] == i) DFS(i);
