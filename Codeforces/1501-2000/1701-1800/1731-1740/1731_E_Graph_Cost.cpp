...
void reset_tc() {
    for (int i = 1; i <= N; i++) ducks[i] = bucks[i] = 0;
}

void solve() {
    cin >> N >> M;
    for (int i = 1; i <= N; i++) {
        for (int factor : nt.divisors[i]) bucks[factor]++;
    }

    // for (int i = 1; i <= N; i++) cout << bucks[i] << " ";
    // cout << endl;

    for (int i = N; i >= 1; i--) bucks[i] = bucks[i] * (bucks[i] - 1) / 2;

    // for (int i = 1; i <= N; i++) cout << bucks[i] << " ";
    // cout << endl;

    for (int i = N; i >= 1; i--) {
        for (int factor : nt.divisors[i]) if (factor != i) bucks[factor] -= bucks[i];
    }
    ...
    ll cost = 0;
    for (int i = N; i >= 2; i--) {
        ll take = min(bucks[i], M - taken) / (i - 1);
        taken += take * (i - 1);
        cost += take * i;
        // cout << i << " taken " << take << endl; 
    }

    if (taken < M) cout << -1 << endl;
    else cout << cost << endl;
