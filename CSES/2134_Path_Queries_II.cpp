...
struct ST
{
    ...
    ll query(int a, int b, int cid, int ss, int se)
    {
        ...
        ll ans = 0;
        int mid = (ss + se) / 2;
        if (a <= mid) maxeq(ans, query(a, b, cid * 2, ss, mid));
        if (b > mid) maxeq(ans, query(a, b, cid * 2 + 1, mid + 1, se));
        return ans;
    }
};
...
ll answer(int a, int b)
{
    ...
    {
        maxeq(ans, chains[aloc].query(loc[a].SS, chainsz[aloc], 1, 1, chainsz[aloc]));
        a = lift[nodes[aloc][chainsz[aloc]]][0];
        aloc = loc[a].FF;
    }
    if (a) maxeq(ans, chains[aloc].query(loc[a].SS, loc[c].SS, 1, 1, chainsz[aloc]));

    while (b && loc[b].FF != loc[c].FF)
    {
        maxeq(ans, chains[bloc].query(loc[b].SS, chainsz[bloc], 1, 1, chainsz[bloc]));
        b = lift[nodes[bloc][chainsz[bloc]]][0];
        bloc = loc[b].FF;
    }
    if (b) maxeq(ans, chains[bloc].query(loc[b].SS, loc[c].SS, 1, 1, chainsz[bloc]));
    return ans;
}
...
int main()
{
    ...
    while (Q--)
    {
        int t, a, b;
        cin >> t >> a >> b;
        if (t == 1) chains[loc[a].FF].update(loc[a].SS, b, 1, 1, chainsz[loc[a].FF]);
        if (t == 2) cout << answer(a, b) << " ";
    }
    cout << endl;
