...
struct ST
{
    ...
    ll query(int a, int b, int cid, int ss, int se)
    {
        ...
        int mid = (ss + se) / 2;
        ll minval = INF;
        if (a <= mid) minval = min(minval, query(a, b, cid * 2, ss, mid));
        if (b > mid) minval = min(minval, query(a, b, cid * 2 + 1, mid + 1, se));
        return minval;
    }
};
...
int main()
{
    ...
    while (Q--)
    {
        int t = read();
        int a = read(), b = read();
        if (t == 1) st.update(a, b, 1, 1, N);
        else cout << st.query(a, b, 1, 1, N) << endl;
    }
    return 0;
}